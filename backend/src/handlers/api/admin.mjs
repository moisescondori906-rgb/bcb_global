...
router.get('/stats', asyncHandler(async (req, res) => {
  const stats = await query(`
    SELECT 
      ...
      (SELECT COALESCE(SUM(COALESCE(saldo_principal, 0) + COALESCE(saldo_comisiones, 0)), 0) FROM usuarios WHERE rol = 'usuario') as balance_total
  `);
...
router.get('/usuarios', asyncHandler(async (req, res) => {
  ...
  const filtered = users.map(u => {
    try {
      ...
      logger.error(`Error sanitizing user ${u.id}: ${error.message}`);
      // Return a basic, safe user object to prevent the entire request from failing
      return {
        id: u.id,
        telefono: u.telefono,
        nombre_usuario: u.nombre_usuario,
        rol: u.rol,
        // Provide default or null values for other fields to prevent further errors
        nombre_real: null,
...
router.get('/retiros', asyncHandler(async (req, res) => {
  try {
    ...
    logger.error(`Error fetching retiros: ${error.message}`);
...
router.get('/device-requests', asyncHandler(async (req, res) => {
  // Placeholder for device requests, return empty array for now
  res.json([]);
}));
