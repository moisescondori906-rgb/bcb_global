...
export function calculateWithdrawalCommissions(monto) {
  const montoSolicitado = Number(monto || 0);
  const comisionOperador = +(montoSolicitado * 0.05).toFixed(2);
  const comisionRetiro = +(montoSolicitado * 0.10).toFixed(2);
  const descuentoTotal = +(comisionOperador + comisionRetiro).toFixed(2);
  const montoNeto = +(montoSolicitado - descuentoTotal).toFixed(2);
  return { montoSolicitado, comisionOperador, comisionRetiro, descuentoTotal, montoNeto };
}

export function formatRetiroMessage(data) {
  const comm = calculateWithdrawalCommissions(data.monto);
  ...
  return `<b>💰 NUEVA SOLICITUD DE RETIRO</b>\n` +
         ...
         `📉 <b>Descuento total 15%:</b> <code>${comm.descuentoTotal} Bs</code>\n` +
         `👨‍💼 <b>Comisión operador 5%:</b> <code>${comm.comisionOperador} Bs</code>\n` +
         `🏦 <b>Comisión retiro 10%:</b> <code>${comm.comisionRetiro} Bs</code>\n` +
